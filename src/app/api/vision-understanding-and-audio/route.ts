...
/**
 * Enhanced endpoint that handles BOTH script formatting AND vision understanding
 * Supports two modes:
 * 1. Script Mode (Legacy): Format script + generate audio
 * 2. Vision Mode (Enhanced): Vision understanding + format concept + generate audio
 * 
 * IMPORTANT: This uses the AUDIO-AWARE vision agent (/api/audio-vision-understanding)
 * which is different from the no-music pipeline vision agent. The audio-aware agent:
 * - Optimizes concepts for powerful narration and TTS delivery
 * - Creates concepts that sound compelling when spoken aloud
 * - Ensures visual complexity doesn't compete with audio narration
 * - Generates emotion arcs that build like spoken stories
 * - Does NOT create timing (Producer Agent handles that after audio transcription)
 */
export async function POST(request: Request) {
  try {
    ...
    console.log(`Processing in ${useVisionMode ? 'VISION' : 'SCRIPT'} mode`);
    ...
    let visionAgentData = null; // Store full vision agent response
    let narrationScript = null; // Store the narration script from vision agent
    let formattedScript = '';
    let processingSteps = [];
    ...
    if (useVisionMode) {
      ...
      if (!concept || !concept.trim()) {
        return NextResponse.json({ error: 'Concept is required for vision mode' }, { status: 400 });
      }
      
      // Step 1: Vision Understanding
      console.log('Step 1: Vision Understanding...');
      const visionStartTime = Date.now();
      
      try {
        // Use AUDIO-AWARE vision understanding agent (different from no-music pipeline)
        ...
        visionAgentData = visionData; // Store the full response for debugging/display
        ...
        console.log(`Vision understanding completed in ${visionTime}s`);
        console.log('Core concept:', visionDocument.core_concept);
        
        // CRITICAL: Check if we have narration script
        narrationScript = visionData.stage1_vision_analysis.narration_script;
        if (!narrationScript) {
          throw new Error('Vision agent did not return narration script for TTS generation');
        }
        
        console.log('Narration script received:', narrationScript.substring(0, 200) + '...');
        
        processingSteps.push({
          name: 'Vision Understanding',
          duration: parseFloat(visionTime),
          success: true
        });
        
        // Step 2: Format the narration script for TTS (remove technical language)
        console.log('Step 2: Formatting narration script for TTS...');
        ...
        processingSteps.push({
          name: 'Narration Script Formatting',
          duration: parseFloat(formatTime),
          success: true
        });
        
      } catch (error) {
        console.error('Vision understanding failed:', error);
        return NextResponse.json({
          error: `Vision understanding failed: ${error instanceof Error ? error.message : 'Unknown error'}`,
          mode: 'vision'
        }, { status: 500 });
      }
      ...
      if (!script || typeof script !== 'string') {
        return NextResponse.json({ error: 'Script is required for script mode' }, { status: 400 });
      }
      
      // Step 1: Format script for TTS
      console.log('Step 1: Formatting script for TTS...');
      const formatStartTime = Date.now();
      ...
      formattedScript = await formatScriptForTTS(script);
      
      const formatTime = ((Date.now() - formatStartTime) / 1000).toFixed(2);
      console.log(`Script formatting completed in ${formatTime}s`);
    ...
    // Step 3: Generate audio from formatted script/concept
    ...
    const response = {
      ...
      folderId,
      stats: {
        scriptLength: useVisionMode ? concept.length : script.length,
    ...
    // Add vision document and raw agent data to response if in vision mode
    if (useVisionMode && visionDocument) {
      response.visionDocument = visionDocument;
      response.visionAgentData = visionAgentData; // Include full agent response
      response.narrationScript = narrationScript; // Include the original narration script
    }
    
    return NextResponse.json(response);
    
  } catch (error) {
    console.error('Error in vision-understanding-and-audio endpoint:', error);
...
/**
 * Format the script for TTS using OpenRouter with Gemini (updated to use OpenRouter)
 */
...
${script}`;
    ...
    console.log('Sending prompt to OpenRouter Gemini (google/gemini-2.5-flash-preview-05-20:thinking)...');
    
    // OpenRouter API call
    const payload = {
      model: "google/gemini-2.5-flash-preview-05-20", // Remove :thinking to avoid meta-commentary
      messages: [
        {
          role: "user",
          content: prompt
