...
**The 8-Segment Prompt Architecture (DIFFUSION MODEL OPTIMIZED):**
Every prompt contains these elements in this EXACT PRIORITY ORDER for optimal diffusion model results:

**CRITICAL DIFFUSION MODEL PRIORITY**: What you mention FIRST gets the most attention from FLUX. Follow this exact order:

1. **SUBJECT & APPEARANCE** (HIGHEST PRIORITY - mentioned first) - Complete character blueprint every time
2. **LOCATION & ENVIRONMENT** (SECOND PRIORITY - mentioned after character) - Extract from DoP location data for consistency
3. **EMOTION & EXPRESSION WITH GAZE** (THIRD PRIORITY) - Micro-expression choreography WITH mandatory gaze direction
4. **POSE & ACTION** - Kinetic moment capture
...
**Location Consistency Protocols:**
- Extract location data from DoP output
- Same location_id = use IDENTICAL location description across all prompts
- Different location_id = use new location description from DoP
- Location description should follow character description for optimal diffusion model attention
...
2. **CRITICAL GAZE RULES**: NEVER "looking at camera" unless explicitly required
3. DEFAULT: "examining [object]", "focused on [task]", "gazing at [environment element]", "looking down at [item]", "staring into distance"
4. Use environment-aware gaze: "gazing out window", "focused on cooking", "watching the sunset"
5. For emotions: "eyes downcast in thought", "looking up hopefully", "glancing sideways nervously"
6. VALIDATE: Every prompt MUST have explicit gaze direction that is NOT camera-focused
7. Include gaze as part of EMOTION & EXPRESSION section
...
[
  "1: Jordan, 20s millennial with tousled chestnut hair and light freckles wearing a white oversized cotton tee with gray sleeve stripe, bedroom at dawn with sunlight through half-drawn blinds and rumpled bedding, brow furrowed and eyes focused on glowing phone screen, in mid-stretch arm reaching for phone, medium wide shot 35 mm, warm morning light key through window and cool blue backlight on phone screen, intimate voyeuristic tension with subtle film grain, 16:9 8K",
  "2: Jordan, 20s millennial with tousled chestnut hair and light freckles wearing a white oversized cotton tee with gray sleeve stripe, modern apartment with organized books and yoga mat on wooden floor, tense jawline with determined expression looking down at metallic lockbox, slow-motion placement of phone into metallic lockbox, medium shot 50 mm, natural sidelight highlighting lockbox engravings, warm inviting tone with soft shadows, 16:9 4K"
]
